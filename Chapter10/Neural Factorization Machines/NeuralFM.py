...
# For the training and the validation error plotting
epoch_list = []
train_err_list = []
valid_err_list = []
test_err_list = []
...
class NeuralFM(BaseEstimator, TransformerMixin):
    ...
    def _initialize_weights(self):
        ...
        if self.pretrain_flag > 0: # with pretrain
            pretrain_file = '/home/asif/pretrain/fm_%s_%d' %(args.dataset, args.hidden_factor, args.dataset, args.hidden_factor)  
    ...
    def train(self, Train_data, Validation_data, Test_data):  # fit a dataset
        ...
        for epoch in range(self.epoch):
            ...
            if self.verbose > 0 and epoch%self.verbose == 0:
                ...
                train_err_list.append(train_result)
                valid_err_list.append(valid_result)
                test_err_list.append(test_result)
...
if __name__ == '__main__':
    ...
    # Plot loss over time
    plt.plot(epoch_list, train_err_list, 'r--', label='NFM training loss per epoch', linewidth=4)
    plt.title('NFM training loss per epoch')
    plt.xlabel('Epoch')
    ...
    plt.plot(epoch_list, valid_err_list, 'r--', label='NFM validation loss per epoch', linewidth=4)
    plt.title('NFM validation loss per epoch')
    plt.xlabel('Epoch')
    ...
    # Model evaluation
    print("RMSE: ")  # evaluate the results for an input set
