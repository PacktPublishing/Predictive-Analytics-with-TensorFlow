...
class FM(BaseEstimator, TransformerMixin):
    ...
    def train(self, Train_data, Validation_data, Test_data):  # fit a dataset
        ...
        for epoch in range(self.epoch):
            ...
            self.test_rmse.append(test_resut)

            if self.verbose > 0 and epoch%self.verbose == 0:
                print("Epoch %d [%.1f s]\ttrain=%.4f, validation=%.4f [%.1f s], test=%.4f [%.1f s]"
...
def make_save_file(args):
    pretrain_path = '/home/asif/pretrain/fm_%s_%d' %(args.dataset, args.hidden_factor)
    if args.mla:
        pretrain_path += '_mla'
...
def train(args):
    ...
    print("Best Iter(validation)= %d\t train = %.4f, valid = %.4f [%.1f s], test = %.4f [%.1f s]" 
...
if __name__ == '__main__':
    ...
    plt.plot(epoch_list, test_err_list, 'r--', label='FM test loss per epoch', linewidth=4)
    plt.title('FM test loss per epoch')
    plt.xlabel('Epoch')
