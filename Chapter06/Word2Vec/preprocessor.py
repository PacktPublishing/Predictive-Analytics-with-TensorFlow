# Text preprocessor: We pull out text helper functions to reduce redundant code
import string
import os
...
def load_movie_data():
    ...
    with open('temp/temp_movie_review_temp.tar.gz', 'wb') as f:
        for chunk in req.iter_content(chunk_size=1024):
            if chunk:
                f.write(chunk)
                f.flush()
    tar = tarfile.open('temp/temp_movie_review_temp.tar.gz', "r:gz")
    tar.extractall(path=save_data_path)
    tar.close()

    pos_file = os.path.join(save_data_path, 'rt-polaritydata', 'rt-polarity.pos')
    neg_file = os.path.join(save_data_path, 'rt-polaritydata', 'rt-polarity.neg')
